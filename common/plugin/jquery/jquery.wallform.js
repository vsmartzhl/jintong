	...
	function fileUpload(a) {
		...
		var httpData = function( xhr, type, s ) { // mostly lifted from jq1.4.4
			...
			if (typeof data === 'string') {
				...
				/*var file_paths = s.imgName.split('\\');
				var img_name = file_paths[file_paths.length-1];

				var reg = /\\.|\/./g;
				var restr = data.replace(/\\/g,"");
				restr = restr.replace(/^\"|\"$/g,'');
				restr = restr.replace(/A/,'{');
				restr = JSON.parse(restr);
				restr.data = restr.data.split('_')[0] + '_' + img_name;
				data  = restr;*/
			}
			return data;
